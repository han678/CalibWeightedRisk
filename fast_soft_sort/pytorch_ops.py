...
def wrap_class(cls, **kwargs):
    ...
    class NumpyOpWrapper(torch.autograd.Function):
        ...
        def forward(ctx, values):
            device = values.device  # Store the original device
            obj = cls(values.cpu().numpy(), **kwargs)  # Move to CPU for NumPy operations
            ctx.numpy_obj = obj
            ctx.device = device  # Save the device for backward pass
            return torch.from_numpy(obj.compute()).to(device)  # Move back to the original device

        @staticmethod
        def backward(ctx, grad_output):
            grad_output_cpu = grad_output.cpu().numpy()  # Move gradients to CPU
            grad_input = ctx.numpy_obj.vjp(grad_output_cpu)  # Compute gradients
            return torch.from_numpy(grad_input).to(ctx.device)  # Move back to the original device
...
def test():
    """Test the soft rank and sort functions."""

    # Test on CPU
    values_cpu = torch.tensor([[8, 0, 5, 3, 2, 1, 6, 7, 9]], dtype=torch.float32)
    print("Soft Rank (CPU):", soft_rank(values_cpu))
    print("Soft Sort (CPU):", soft_sort(values_cpu))

    # Test on GPU
    if torch.cuda.is_available():
        values_gpu = values_cpu.to("cuda")
        print("Soft Rank (GPU):", soft_rank(values_gpu))
        print("Soft Sort (GPU):", soft_sort(values_gpu))
