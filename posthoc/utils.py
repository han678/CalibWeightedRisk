...
from loss.aurc import get_score_function, zero_one_loss, cross_entropy_loss
...
def evaluate_metacal_metrics(X_train, y_train, X_test, y_test, score_function="MSP", method="miscoverage", acc=0.85, alpha=0.05):
    ...
    loss = zero_one_loss(probs, y_test_onehot)
...
def get_logits_file_paths(args):
    if args.loss_type == "aurc":
        file_name = f"{args.arch}_{args.loss_type}_{args.score_function}"
    else:
        file_name = f"{args.arch}_{args.loss_type}"
    npz_folder = os.path.join("train_output", args.dataset, "npz",  f"seed{args.seed}")
    train_logits_file = os.path.join(npz_folder, f"{file_name}_train_logits.npz")
    test_logits_file = os.path.join(npz_folder, f"{file_name}_test_logits.npz")
    return train_logits_file, test_logits_file
