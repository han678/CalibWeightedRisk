...
class ECE_KDELoss(torch.nn.Module):
    """
    ...
    def forward(self, input: Tensor, target: Tensor) -> Tensor:
        ...
        with torch.no_grad():
            bandwidth = get_bandwidth(f=scores, device=input.device).to(input.device)
        kde_loss = get_ece_kde(scores, target, bandwidth=bandwidth, p=self.p, mc_type=self.mc_type, device=input.device)
        ce_loss = self.criterion(input, target)
        return ce_loss + self.gamma * kde_loss
...
def get_bandwidth(f, device):
    ...
    max_b = -1
    max_l = 0
    n = torch.tensor(len(f), device=device)
    for b in bandwidths:
...
def get_ece_kde(f, y, bandwidth, p, mc_type, device):
    ...
    # Convert one-hot encoded labels to target labels
    if y.dim() > 1 and y.size(1) > 1:
        y = torch.argmax(y, dim=1)

    check_input(f, bandwidth)
    if f.shape[1] == 1:
        return 2 * get_ratio_binary(f, y, bandwidth, p, device).item()
    else:
        if mc_type == 'canonical':
            return get_ratio_canonical(f, y, bandwidth, p, device).item()
        elif mc_type == 'marginal':
            return get_ratio_marginal_vect(f, y, bandwidth, p, device).item()
        elif mc_type == 'top_label':
            return get_ratio_toplabel(f, y, bandwidth, p, device).item()
